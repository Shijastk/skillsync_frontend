...
const RightSidebar = ({ trendingSkills, incomingRequests }) => {
  return (
    <aside className="hidden lg:flex flex-col gap-6 sticky top-24">
            ...
            {incomingRequests.map((req) => (
                <div key={req.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    ...
                    <div className="flex gap-2">
                        <button className="flex-1 bg-gray-900 text-white text-xs font-semibold py-1.5 rounded-lg hover:bg-gray-800 transition-colors flex items-center justify-center gap-1">
                            <Check size={14} /> Accept
                        </button>
                        <button className="flex-1 bg-white border border-gray-200 text-gray-700 text-xs font-semibold py-1.5 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-1">
                            <X size={14} /> Decline
                        </button>
                    </div>
                </div>
      ...
      {/* 2. Targeted Mentors */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <h3 className="font-bold text-gray-900 text-sm mb-4">Top Mentors for You</h3>
        <div className="space-y-3">
             {[
                { name: "Lisa Wang", role: "Rust Expert", rating: 5.0, img: "LW", skill: "Rust" },
                { name: "James Kim", role: "Go Developer", rating: 4.8, img: "JK", skill: "Go" },
             ].map((mentor, idx) => (
                 <div key={idx} className="flex items-center justify-between group cursor-pointer p-2 -mx-2 hover:bg-gray-50 rounded-xl transition-colors">
                     <div className="flex items-center gap-3">
                         ...
                         <div>
                             ...
                             <div className="text-xs text-gray-500 flex items-center gap-1">
                                ...
                                {mentor.rating} • Teaches {mentor.skill}
                             </div>
                         </div>
                     </div>
